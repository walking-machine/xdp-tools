...
static struct prog_option udp_options[] = {
	...
	DEFINE_OPTION("src-addr", OPT_IPADDR, struct udpopt, src_ip,
		      .short_opt = 'A',
		      .metavar = "<addr>",
		      .help = "Source address of generated packets"),
	...
	DEFINE_OPTION("dyn-ports", OPT_U16, struct udpopt, dyn_ports,
		      .short_opt = 'd',
		      .metavar = "<num ports>",
		      .help = "Enable dynamic port Source port of generated packets"),
...
static struct prog_option tcp_options[] = {
	DEFINE_OPTION("dst-port", OPT_U16, struct tcpopt, dst_port,
		      .short_opt = 'p',
		      .metavar = "<port>",
		      .help = "Connect to port <port> of destination"),
	DEFINE_OPTION("num-packets", OPT_U32, struct tcpopt, num_pkts,
		      .short_opt = 'n',
		      .metavar = "<port>",
		      .help = "Number of packets to send"),
	DEFINE_OPTION("threads", OPT_U16, struct tcpopt, threads,
		      .short_opt = 't',
		      .metavar = "<threads>",
		      .help = "Number of simultaneous threads to transmit from"),
...
int do_tcp(const void *opt, __unused const char *pin_root_path)
{
	...
	if (cfg->threads > 1) {
		pr_warn("Multiple threads not supported yet\n");
		goto out;
	}
	...
	err = create_runners(&runner_threads, &t, cfg->threads, &tcfg, test_prog);
	if (err)
		goto out;
